...
# The integration branch stores conflict-heavy porting as explicit audited
# patches. Materialize them before any invariant test or build. Upstream #1281
# must receive flattened source only after the fork gate is 5/5 green.
if ! grep -Fq 'IsPQHybridActiveLocked' src/validation.cpp; then
  chmod +x contrib/devtools/apply-rip25-v48-port.sh
  ./contrib/devtools/apply-rip25-v48-port.sh
fi
